...
window.AmeVisibleUserDialog = (function($, _) {
	...
	function removeSelectedUser(user, tableRow) {
		...
		if (user.userLogin === currentUserLogin) {
			return;
		}
	...
	function updateSearchResults() {
		loadedUsers = _(loadedUsers)
			...
			.sortByOrder(['searchScore', 'user_login'], ['desc', 'asc'])
	...
	function calculateSearchScore(user, query, keywords) {
		...
		var haystack = user.user_login.toLowerCase() + '\n' + user.display_name.toLowerCase();
	...
	function buildTableRow(user, action) {
		...
		return $('<tr></tr>')
			...
			.toggleClass('ws_is_current_user', user.userLogin === currentUserLogin)
			.append($('<td></td>', {
	...
	$(function() {
		...
		$searchBox.on('change keyup input paste click propertychange ', _.debounce(function() {
			...
			newQuery = AmeEditorApi.jsTrim(newQuery.toLowerCase().replace(/\s{2,}/, ' '));
	...
	return {
		...
		open: function(options) {
			...
			var knownUsers = options.users,
				visibleUsers = [].concat(options.visibleUsers); //Don't modify the input array.

			//Always include the current user.
			if (!_.includes(visibleUsers, currentUserLogin)) {
				visibleUsers.unshift(currentUserLogin);
			}

			selectedUsers = _.map(visibleUsers, function(login) {
