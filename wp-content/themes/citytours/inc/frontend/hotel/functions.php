...
if ( ! function_exists( 'ct_hotel_calc_room_price' ) ) {
	function ct_hotel_calc_room_price( $hotel_id, $room_type_id, $from_date, $to_date, $rooms=1, $adults=1, $kids=0) {
		...
		foreach ( $period as $dt ) {
			...
			$sql = "SELECT vacancies.room_type_id, vacancies.price_per_room , vacancies.price_per_person, vacancies.price_per_child, vacancies.weekend_price_fri, vacancies.weekend_price_sat, vacancies.peakseason_price
					FROM (SELECT room_type_id, rooms, price_per_room, price_per_person, price_per_child,weekend_price_fri,weekend_price_sat,peakseason_price
							FROM " . CT_HOTEL_VACANCIES_TABLE . " 
			...
			if ( empty( $result ) ) { //if no available rooms on selected date
				...
				$todayName = date("D", strtotime($check_date));
				$nowYear = date("n", strtotime($check_date));
				if($todayName == 'Fri' && $result->weekend_price_fri !== '0.00') $day_price = $result->weekend_price_fri;
				if($todayName == 'Sat' && $result->weekend_price_sat !== '0.00') $day_price = $result->weekend_price_sat;
				if($nowYear >= 4 && $nowYear <= 10 && $result->peakseason_price !== '0.00') $day_price = $result->peakseason_price;
				...
				/*if(date('D') == 'Fri' && $result->weekend_price_fri !== '0.00') $total_price = $result->weekend_price_fri;
				if(date('D') == 'Sat' && $result->weekend_price_sat !== '0.00') $total_price = $result->weekend_price_sat;
				if(date('n') >= 4 && date('n') <= 10 && $result->peakseason_price !== '0.00') $total_price = $result->peakseason_price;	*/	
				if(!empty($get_discount_price)){
					$discountPrice = 100-$get_discount_price;
					$discountedPrice = ($discountPrice*$total_price)/100;
				} else {
				...
				/*$bedPrice = 0;
				if($extraBed > 0){
					$bedPrice = $extraBed*$extra_bed_no;
					$discountedPrice = $discountedPrice + $bedPrice;
				}*/

		$return_value = array(
			'check_dates' => $check_dates,
			'prices'      => $price_data,
			'total_price' => round($discountedPrice),
		);
...
if ( ! function_exists( 'ct_hotel_get_search_result_count' ) ) {
	function ct_hotel_get_search_result_count( $args ) {
		...
		if ( $by == 'star' ) {
			...
			// hotel_location filter
			if ( ! empty( $hotel_location ) && trim( implode( '', $hotel_location ) ) != "" ) {
				...
				$where .= " AND tt.taxonomy = 'hotel_location' AND tt.term_id IN (" . esc_sql( implode( ',', $hotel_location ) ) . ")";
			}
...
if ( ! function_exists( 'ct_hotel_get_special_hotels' ) ) {
	function ct_hotel_get_special_hotels( $type='latest', $count=6, $exclude_ids, $hotel_location=array() ) {
		$args = array(
				'post_type'  => 'hotel',
				'suppress_filters' => 0,
		...
		if ( ! empty( $hotel_location ) ) {
			if ( is_numeric( $hotel_location[0] ) ) {
		...
		if ( $type == 'featured'  ) {
			...
			if ( ! empty( $hotel_location ) ) {
				$sql .= " INNER JOIN {$tbl_term_relationships} AS tr ON tr.object_id = t1.hotel_id 
						INNER JOIN {$tbl_term_taxonomy} AS tt ON tt.term_taxonomy_id = tr.term_taxonomy_id";
				$where .= " AND tt.taxonomy = 'hotel_location' AND tt.term_id =" . esc_sql( $hotel_location ) . ")";
			}
			...
			if ( count( $popular_hotel ) < $count ) {
				...
				$result = array_merge( $result, ct_hotel_get_special_hotels( 'latest', $count - count( $popular_hotel ), $exclude_ids, $hotel_location ) );
			}
			return $result;
		}
	}
