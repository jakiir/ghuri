...
$search_result = ct_tour_get_search_result( array( 's'=>$s, 'date'=>$date, 'adults'=>$adults, 'kids'=>$kids, 'tour_type'=>$tour_type, 'tour_location'=>$locations, 'price_filter'=>$price_filter, 'rating_filter'=>$rating_filter, 'facility_filter'=>$facility_filter, 'order_by'=>$order_by_array[$order_by], 'order'=>$order, 'last_no'=>( $page - 1 ) * $per_page, 'per_page'=>$per_page ) );
$post_list = $search_result['ids'];
$count = $search_result['count']; // total_count

$header_img_scr = ct_get_header_image_src('tour');

$get_tour_location_img = '';
if($_REQUEST['locations'] !=''){
	$locationss = $_REQUEST['locations'];
	if(is_array($locationss)){
		$locationId = $locationss[0];
	}else{
		$locationId = $locationss;
	}	
	$options = "tour_location_{$locationId}_images";
	$get_tour_location_img = get_site_option( $options );
	$header_img_scr_location = esc_url( wp_get_attachment_url( $get_tour_location_img ) );
	$header_img_scr = ($header_img_scr_location !='' ? $header_img_scr_location : $header_img_scr);
}
...
<div class="container margin_60">
	<div class="row">
				...
				<div class="modify_search_wp">
					<form role="search" method="get" id="search-tour-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
						...
						<div class="form-group">
							<label><?php echo esc_html( 'Location', 'citytours' ) ?></label>
						...
						<div class="form-group">
							<label><?php echo esc_html( 'Themes', 'citytours' ) ?></label>
							<?php
						...
						<div class="form-group">
							...
							<div class="numbers-row">
								...
								<input type="text" value="<?php echo $adults; ?>" class="qty2 form-control" name="adults" value="<?php echo esc_attr( $adults ) ?>">
							</div>
						</div>
						<div class="form-group add_bottom_30">
							<label><?php esc_html_e( 'Children', 'citytours' ) ?></label>
							<div class="numbers-row">
								<?php $kids = ( isset($_GET['kids']) ? $_GET['kids'] : 0 ); ?>
								<input type="text" value="<?php echo $kids; ?>" class="qty2 form-control" name="kids" value="<?php echo esc_attr( $kids ) ?>">
							</div>
						</div>
		...
		<div id="locations_search">
			<a data-toggle="collapse" href="#collapseLocations_search" aria-expanded="false" aria-controls="collapseLocations_search" id="modify_col_bt"><i style="font-size:20px;" class="icon-location-5"></i><?php echo esc_html( 'Locations', 'citytours' ) ?> <i class="icon-plus-1 pull-right"></i></a>
			<div class="collapse" id="collapseLocations_search">
				<div class="Locations_search_wp">
					<?php if ( ! empty( $ct_options['tour_location_filter'] ) ) :?>
					<div class="filter_type">
						<h6><?php echo esc_html__( 'Locations', 'citytours' ) ?></h6>
						<ul class="list-filter location-filter" data-base-url="<?php echo esc_url( remove_query_arg( array( 'locations', 'page' ) ) ); ?>" data-arg="locations">
							<?php 
							$all_locations = get_terms( 'tour_location', array('hide_empty' => 0) );
							if ( ! empty( $all_locations ) ) :
							foreach ( $all_locations as $location ) {
								$term_id = $location->term_id;
								$checked = ( in_array( $term_id, $locations ) ) ? ' checked="checked"' : '';
								echo '<li><label><input type="checkbox" name="tour_location_filter[]" value="' . esc_attr( $term_id ) . '"' . $checked . '>' . esc_html( $location->name ) . '</label></li>';
							}
							endif;?>
						</ul>
					</div>
			...
			<div class="collapse" id="collapseFilters">
				...
				if ( ! empty( $ct_options['tour_price_filter'] ) ) :?>
					...
					array_unshift($step_arr, 0);
					 ?>
					<div class="filter_type">
						<h6><?php echo esc_html__( 'Price (BDT)', 'citytours' ) ?></h6>
						<p id="amount"></p>
						<div id="tour-price-range"></div>
						<p id="priceFileterItem" style="margin-top:15px;">						  
							<input type="hidden" name="price_filter[]" value="0" id="amount1">
							<input type="hidden" name="price_filter[]" value="1" id="amount2">												 
						</p>
				...
				<div class="filter_type">
					<h6><?php echo esc_html__( 'Property Type', 'citytours' ) ?></h6>
					<ul class="list-filter property_type-filter" data-base-url="<?php echo esc_url( remove_query_arg( array( 'property_type_filter', 'page' ) ) ); ?>" data-arg="property_type_filter">
						
					<li><label><input type="checkbox" name="property_type_filter[]" value=""><span class="property_type">Hotel</span></label></li>
					<li><label><input type="checkbox" name="property_type_filter[]" value=""><span class="property_type">Resort</span></label></li>
					<li><label><input type="checkbox" name="property_type_filter[]" value=""><span class="property_type">Cruise Ship</span></label></li>
		...
		<!--<div class="box_style_cat">
			<ul id="cat_nav">
				<?php
					...
					echo '<li class="all-types"><a href="' . esc_url( remove_query_arg( array( 'tour_types', 'page' ) ) ) . '"' . $selected . '><i class="icon_set_1_icon-51"></i>' . esc_html__( 'All themes', 'citytours' ) . '<small>(' . esc_html( array_sum( $counts_by_tour_type ) ) . ')</small></a></li>';
					$all_tour_types = get_terms( 'tour_type', array('hide_empty' => 0) );
					if ( ! empty( $all_tour_types ) ) :
...
jQuery(document).ready(function(){
	...
	var maxPrice = "<?=end($step_arr);?>";
	var secondPrice = "<?=$step_arr[1];?>";
	jQuery( "#tour-price-range" ).slider({
		  range: true,
		  min: 0,
		  max: maxPrice,
		  step: 2000,
		  ...
		  slide: function( event, ui ) {
			jQuery( "#amount" ).html( '<span class="price01">' + ui.values[ 0 ] + "</span><span class='price02'>" + ui.values[ 1 ] + '</span>' );			
			jQuery( "#amount1" ).val(ui.values[ 0 ]);
			jQuery( "#amount2" ).val(ui.values[ 1 ]);
		  },
		  stop: function( event, ui ) {
			/*if( 0 <= ui.values[ 0 ] && secondPrice >= ui.values[ 1 ]){
				console.log(secondPrice+'---'+ui.values[ 1 ]);
			}*/
		  }
		});
	jQuery( "#amount" ).html( '<span class="price01">' + jQuery( "#tour-price-range" ).slider( "values", 0 ) + "</span><span class='price02'>" + jQuery( "#tour-price-range" ).slider( "values", 1 ) + '</span>' );
