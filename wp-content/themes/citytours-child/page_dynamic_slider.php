<?php
 /*
 Template Name: Page dynamic slider
 */
	...
	while ( have_posts() ) : the_post();
		...
		if ( class_exists( 'RevSlider' ) && $slider != 'Deactivated' ) {
			...
			if(!empty($_SESSION['cart'])){
				$hotelId = (!empty($_SESSION['cart'][$_GET['uid']]['hotel_id']) ? $_SESSION['cart'][$_GET['uid']]['hotel_id'] : '');
				if($hotelId == ''){
					$hotelId = (!empty($_SESSION['cart'][$_GET['uid']]['tour_id']) ? $_SESSION['cart'][$_GET['uid']]['tour_id'] : '');
				}
				if($hotelId == ''){
					if(isset($_GET['tour_id'])){
						$hotelId = $_GET['tour_id'];
					} else {
						$hotelId = $post_id;
					}					
				}						
			...
			if(isset($_REQUEST['booking_no']) && isset($_REQUEST['pin_code'])){
				$order_cus = new CT_Hotel_Order( $_REQUEST['booking_no'], $_REQUEST['pin_code'] );
				$order_data_cus = $order_cus->get_order_info();
				$hotelId = $order_data_cus['post_id'];	
			}
			
			
			$header_img_scr = ct_get_header_image_src( $hotelId );
