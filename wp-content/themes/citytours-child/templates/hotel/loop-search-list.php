...
$hotel_price = get_post_meta( $post_id, '_hotel_price', true );
		...
		<div class="col-lg-5 col-md-5 col-sm-5">
			<div class="tour_list_desc">
				...
				echo '<span class="capacityIcon">';
				for($i=0;$i<$get_person;$i++){
					echo '<i class="icon-user"></i>';
				}
				echo '</span>';
				echo '<span class="roomType">';
				if(isset($_GET['rooms'])){
					echo '<h4>'.esc_html( get_the_title( $room_type_id ) ).'</h4>';
				}
				echo '</span>';
				?>											
			</div>
		...
		<div class="col-lg-3 col-md-3 col-sm-3">
			<div class="price_list">
				...
				<?php $hotel_priced = (!empty($hotel_price) ? $hotel_price : $room_price); ?>	
					...
					<p><a href="<?php echo esc_url( $hotelDetails ); ?>" class="btn btn-56"><?php echo esc_html__( 'Book now', 'citytours' ) ?></a></p>
				</div>
			</div>
		</div>
	</div>
