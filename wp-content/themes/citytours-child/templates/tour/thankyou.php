...
$deposit_rate = get_post_meta( $order_data['post_id'], '_tour_security_deposit', true ); $deposit_rate = empty( $deposit_rate ) ? 0 : $deposit_rate;
...
<div class="row">
	<div class="col-md-8">
		...
		<div class="step">
			...
			<?php endif; ?>
			<tr>
				<td><strong><?php echo esc_html__( 'Tour Name', 'citytours' ) ?></strong></td>
			...
			<tr>
				...
				<td ><?php echo ct_price( $order_data['total_price'] ) ?> BDT</td>
			</tr>
			</tbody>
			</table>
		</div><!--End step -->
	...
	<div class="box_style_1">
		...
		<?php if ( ! empty( $ct_options['tour_thankyou_text_2'] ) ) : ?>
		<p><?php echo esc_html__( $ct_options['tour_thankyou_text_2'], 'citytours' ) ?></p>
		<?php endif; ?>
		<hr>
		<?php if ( ! empty( $ct_options['tour_invoice_page'] ) ) : ?>
			<!--<a class="btn_full_outline" target="_blank" href="<?php //echo esc_url( add_query_arg( array( 'booking_no' => $order_data['booking_no'], 'pin_code' => $order_data['pin_code'] ) ,ct_get_permalink_clang( $ct_options['tour_invoice_page'] ) ) ) ?>" target="_blank"><?php //echo esc_html__( 'View your invoice', 'citytours' ) ?></a>-->
			<a class="btn_full_outline" target="_blank" href="<?php echo esc_url( add_query_arg( array( 'booking_no' => $order_data['booking_no'], 'pin_code' => $order_data['pin_code'] ) ,home_url('/tour-booking-confirmation/') ) ) ?>" target="_blank"><?php echo esc_html__( 'View your confirmation', 'citytours' ) ?></a>
		<?php endif; ?>
	</div>
	</aside>
</div><!--End row -->